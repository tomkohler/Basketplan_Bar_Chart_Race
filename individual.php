...
foreach($linkcache as $name=>$value) {	
    ...
    //file_put_contents($dir3.'test.txt', $picturelink2, FILE_APPEND);
	...
	$counter=0;
	$temp=$dir2.$title."-".utf8_encode($playername).".png";
	foreach (glob($temp) as $fileXname) {
		$counter += 1;
	}
	
	if ($counter > 0) {
		echo '<a href="imgshow.php?f='.utf8_encode($filename).'"><img class="item" src="'.utf8_encode($picturelink2).'" alt="'.utf8_encode($playername).'"><figcaption class="fc">'.utf8_encode($playername).'</figcaption></a>';
	}
}
echo '</div>';
?>	
